                ...
                @if($activeTab === 'shop_details')
                    <div>
                        @if($showShopManager)
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-4">
                                    <h2 class="text-xl font-semibold text-gray-900">Shop Manager</h2>
                                    <button
                                        wire:click="closeShopManager"
                                        class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-gray-700 uppercase transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25"
                                    >
                                        <svg class="h-4 w-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <livewire:shop.shop-manager :key="'shop-manager-'.$selectedShopId" />
                            </div>
                        @else
                            <!-- Shop Selection and Overview -->
                            <div class="mb-8 overflow-hidden bg-white rounded-lg shadow-lg">
                                <div class="p-6">
                                    <div class="flex items-center justify-between mb-6">
                                        <h2 class="text-xl font-semibold text-gray-900">My Shops</h2>

                                        @if($selectedShopId)
                                            <button
                                                wire:click="toggleShopManager({{ $selectedShopId }})"
                                    ...
                                    <div class="mb-6">
                                        <label for="shop-selector" class="block text-sm font-medium text-gray-700">Select Shop</label>
                                        <select
                                            id="shop-selector"
                                    ...
                                    <!-- Shop Details Content if a shop is selected -->
                                    @if($selectedShopId && $selectedShop)
                                        <div class="mt-6">
                                            <div class="flex items-center mb-6 space-x-4">
                                                <div class="flex-shrink-0">
                                                    @if($selectedShop->image)
                                                        <img src="{{ Storage::url($selectedShop->image) }}" alt="{{ $selectedShop->name }}" class="object-cover w-16 h-16 rounded-full">
                                                    @else
                                                ...
                                                <div>
                                                    <h3 class="text-lg font-medium text-gray-900">{{ $selectedShop->name }}</h3>
                                                    <div class="flex items-center space-x-2 text-sm text-gray-500">
                                                        <span>Status: </span>
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                            @if($selectedShop->status === 'approved') bg-green-100 text-green-800
                                                            @elseif($selectedShop->status === 'pending') bg-yellow-100 text-yellow-800
                                                            @else bg-red-100 text-red-800 @endif">
                                                            {{ ucfirst($selectedShop->status) }}
                                                        </span>

                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                            {{ $selectedShop->is_active ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800' }}">
                                            ...
                                            <div class="border-b border-gray-200">
                                                <nav class="flex -mb-px space-x-8" aria-label="Tabs">
                                                    <button
                                                        wire:click="setShopDetailView('overview')"
                                                        class="{{ $shopDetailView === 'overview' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"
                                                    >
                                                        Overview
                                                    </button>
                                                    <button
                                                        wire:click="setShopDetailView('inventory')"
                                                        class="{{ $shopDetailView === 'inventory' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"
                                                    >
                                                        Inventory
                                                    </button>
                                                    <button
                                                        wire:click="setShopDetailView('categories')"
                                                        class="{{ $shopDetailView === 'categories' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"
                                                    >
                                                        Categories
                                                    </button>
                                                    <button
                                                        wire:click="toggleShopManager"
                                                        class="{{ $shopDetailView === 'manager' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"
                                                    >
                                                        Shop Manager
                                                    </button>
                                                </nav>
                                        ...
                                        <!-- Shop Detail Content Based on Selected View -->
                                        <div class="mt-6">
                                            @if($shopDetailView === 'overview')
                                                ...
                                                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                                            ...
                                                            <div class="mt-5 border-t border-gray-200">
                                                                <dl class="sm:divide-y sm:divide-gray-200">
                                                                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                                                                        ...
                                                                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $selectedShop->name }}</dd>
                                                                    </div>
                                                                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                                                                        <dt class="text-sm font-medium text-gray-500">Address</dt>
                                                                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $selectedShop->address ?: 'Not set' }}</dd>
                                                                    </div>
                                                                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                                                                        <dt class="text-sm font-medium text-gray-500">Phone</dt>
                                                                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $selectedShop->phone ?: 'Not set' }}</dd>
                                                                    </div>
                                                                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                                                                        <dt class="text-sm font-medium text-gray-500">Email</dt>
                                                                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $selectedShop->email ?: 'Not set' }}</dd>
                                                                    </div>
                                                                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                                                    ...
                                                    <div class="overflow-hidden bg-white border border-gray-200 rounded-lg shadow">
                                                        <div class="px-4 py-5 sm:p-6">
                                                            <h3 class="text-lg font-medium leading-6 text-gray-900">Shop Metrics</h3>
                                                            <div class="grid grid-cols-1 gap-5 mt-5 sm:grid-cols-2">
                                                                <div class="overflow-hidden bg-white border border-gray-200 rounded-lg shadow-sm">
                                                                    <div class="px-4 py-5 sm:p-6">
                                                                        <dt class="text-sm font-medium text-gray-500 truncate">Total Products</dt>
                                                                        <dd class="mt-1 text-3xl font-semibold text-gray-900">{{ $userShopProducts }}</dd>
                                                                    </div>
                                                                </div>
                                                                <div class="overflow-hidden bg-white border border-gray-200 rounded-lg shadow-sm">
                                                                    <div class="px-4 py-5 sm:p-6">
                                                                        <dt class="text-sm font-medium text-gray-500 truncate">Total Categories</dt>
                                                                        <dd class="mt-1 text-3xl font-semibold text-gray-900">{{ is_array($shopCategories) ? count($shopCategories) : $shopCategories->count() }}</dd>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            @elseif($shopDetailView === 'inventory')
                                                ...
                                                <div>
                                                    <h3 class="mb-4 text-lg font-medium leading-6 text-gray-900">Shop Inventory</h3>

                                                    @if(count($productsByCategory) > 0)
                                                        <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
                                                            <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm md:col-span-1">
                                                                <h4 class="mb-3 font-medium text-gray-900">Categories</h4>
                                                                <nav class="space-y-1" aria-label="Sidebar">
                                                                    @foreach($productsByCategory as $categoryId => $categoryData)
                                                                        <button
                                                                            wire:click="selectCategory({{ $categoryId }})"
                                                                            class="w-full flex items-center px-3 py-2 text-sm font-medium rounded-md {{ $selectedCategoryId == $categoryId ? 'bg-indigo-100 text-indigo-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}"
                                                                        >
                                                                            {{ $categoryData['category_name'] }}
                                                                            <span class="ml-auto inline-block py-0.5 px-2 text-xs rounded-full {{ $selectedCategoryId == $categoryId ? 'bg-indigo-200' : 'bg-gray-100' }}">
                                                                                {{ count($categoryData['products']) }}
                                                                            </span>
                                                                        </button>
                                                                    @endforeach
                                                                </nav>
                                                            ...
                                                            <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm md:col-span-3">
                                                                @if($selectedCategoryId && isset($productsByCategory[$selectedCategoryId]))
                                                                    <h4 class="mb-3 font-medium text-gray-900">{{ $productsByCategory[$selectedCategoryId]['category_name'] }} Products</h4>

                                                                    @if(count($productsByCategory[$selectedCategoryId]['products']) > 0)
                                                                ...
                                                                @else
                                                                    <div class="flex justify-center py-8">
                                                                        <p class="text-center text-gray-500">Select a category to view products</p>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    @else
                                                        <div class="flex justify-center p-8 bg-white border border-gray-200 rounded-lg shadow-sm">
                                                            <div class="text-center">
                                                                <svg class="w-12 h-12 mx-auto text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                                                                </svg>
                                                                <h3 class="mt-2 text-sm font-medium text-gray-900">No products found</h3>
                                                                <p class="mt-1 text-sm text-gray-500">Get started by adding products to your shop.</p>
                                                                <div class="mt-6">
                                                                    <button
                                                                        wire:click="toggleShopManager"
                                            ...
                                            @elseif($shopDetailView === 'categories')
                                                ...
                                                <div>
                                                    <div class="flex items-center justify-between mb-4">
                                                        <h3 class="text-lg font-medium leading-6 text-gray-900">Shop Categories</h3>
                                                        <button
                                                            wire:click="toggleShopManager"
                                                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                                        >
                                                                    ...
                                                                    <li class="p-4 hover:bg-gray-50">
                                                                        <div class="flex items-center justify-between">
                                                                            <div>
                                                                                ...
                                                                                <p class="mt-1 text-xs text-gray-500">{{ $category['description'] ?? 'No description' }}</p>
                                                                            </div>
                                                                            <div class="flex items-center">
                                                                                ...
                                                                                <span class="inline-flex px-2 ml-3 text-xs font-semibold leading-5 text-blue-800 bg-blue-100 rounded-full">
                                                                        ...
                                                                        @if(count($category['children']) > 0)
                                                                            <ul class="pl-4 mt-3 space-y-2 border-l-2 border-gray-100">
                                                                                @foreach($category['children'] as $childId => $child)
                                                                                    <li class="py-1">
                                                                                        <div class="flex items-center justify-between">
                                                                                            <div>
                                                                                                <h5 class="text-xs font-medium text-gray-700">{{ $child['name'] }}</h5>
                                                                                            </div>
                                                                                            <div class="flex items-center">
                                                                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $child['status'] === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                    ...
                                    @else
                                        <div class="flex justify-center p-8 mt-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                                            <div class="text-center">
                                                <svg class="w-12 h-12 mx-auto text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                                                </svg>
                                                <h3 class="mt-2 text-sm font-medium text-gray-900">No shop selected</h3>
                                                <p class="mt-1 text-sm text-gray-500">Select a shop from the dropdown above to view its details.</p>
                                            </div>
                                        </div>
                                    @endif
                                </div>
